...
def plt_house_x(X, y,f_wb=None, ax=None):
    ...
    ax.set_ylabel('Price (in 2000s of dollars)')
    ax.set_xlabel(f'Size (2000 sqft)')
    if f_wb is not None:
...
def plt_intuition(x_train, y_train):
    ...
    w_range = np.array([2000-2000,2000+2000])
    tmp_b = 100
...
def plt_contour_wgrad(x, y, hist, ax, w_range=[-100, 2000, 5], b_range=[-2000, 2000, 5],
                contours = [0.1,50,2000,20000,20000,220000,200000],
                      resolution=5, w_final=2000, b_final=100,step=10 ):
...
def plt_divergence(p_hist, J_hist, x_train,y_train):
    ...
    fix_b = 100
    w_array = np.arange(-70000, 70000, 2000)
    cost = np.zeros_like(w_array)
    ...
    tmp_b,tmp_w = np.meshgrid(np.arange(-320000, 320000, 2000),np.arange(-70000, 70000, 2000))
    z=np.zeros_like(tmp_b)
    for i in range(tmp_w.shape[0]):
        for j in range(tmp_w.shape[1]):
            z[i][j] = compute_cost(x_train, y_train, tmp_w[i][j], tmp_b[i][j] )
...
def plt_gradients(x_train,y_train, f_compute_cost, f_compute_gradient):
    ...
    # Print w vs cost to see minimum
    fix_b = 100
    w_array = np.linspace(-100, 2000, 50)
    ...
    tmp_b,tmp_w = np.meshgrid(np.linspace(-2000, 2000, 10), np.linspace(-100, 600, 10))
